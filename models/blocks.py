...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, c_in, c_out, relu_type='prelu', norm_type='bn', scale='none', hg_depth=2, att_name='mrr',isPrint=False):
...
class HourGlassBlock(nn.Module):
    ...
    def _generate_network(self, level):
        ...
        self.add_module('b1_' + str(level), Inception_Module(c1, c2))
...
class Inception_Module(nn.Module):  # try reflection pad
    def __init__(self, in_c, out_c):
        ...
        # self.feature_extract_5 = nn.Sequential(
        #     nn.ReflectionPad2d(1),
        #     nn.Conv2d(in_c, out_c // 2, 3, 1),
        #     nn.ReflectionPad2d(1),
        #     nn.Conv2d(out_c//2, out_c//4, 3, 1),
        # )

        #尝试使用空洞卷积代替5X5 K=k+(k-1)*(r-1)
        # self.feature_extract_5 = nn.Sequential(
        #     nn.ReflectionPad2d(2),
        #     nn.Conv2d(in_c,out_c//4,3,1,0,2) #dialation=2
        # )

        #尝试使用空洞卷积代替7X7 K=k+(k-1)*(r-1)
        # self.feature_extract_7 = nn.Sequential(
        #     nn.ReflectionPad2d(3),
        #     nn.Conv2d(in_c,out_c//4,4,1,0,2) #dialation=2
        # )
