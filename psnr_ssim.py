...
def NRMSE(im1, im2):
    im1, im2 = rgb2y_matlab(im1), rgb2y_matlab(im2)
    im1, im2 = im1.astype(np.float), im2.astype(np.float)
    mse = np.mean(np.square(im1 - im2))
    # print(mse)
    nrmse = np.sqrt(mse)
    return nrmse
...
def psnr_ssim_dir(gt_dir, test_dir):
    ...
    for gt_name, test_name in zip(gt_img_list, test_img_list):
        ...
        NRMSE_score += NRMSE(gt_img, test_img)
        i=i+1
    print(i)
    return psnr_score / len(gt_img_list), ssim_score / len(gt_img_list), NRMSE_score / len(gt_img_list)
...
if __name__ == '__main__':
    ...
    test_dirs = [
        # '../results_helen/SPARNet_S16_V4_Attn2D_trainByInceptionModule_noAttentionBranch_useInstanceNorm',
        # '/home/ubuntu/HDD2T/HWK/compare result/DIC/Helen/',
        # '/home/ubuntu/HDD2T/HWK/PerceptualSimilarity-master/Helen/',
        './results_CelebA/SPARNet_S16_V4_Attn2D_trainByInceptionModule_noAttentionBranch',
        # '../Deep-Iterative-Collaboration/results/DIC_in3f48_x8_CelebA/CelebA'
    ]
    # gt_dir = '/home/ubuntu/HDD2T/HWK/helen_result/HR'
    # gt_dir = './test_dirs/Helen_test_DIC/HR'
    # gt_dir = gt_dir.replace("\\", "\/")
    # print(gt_dir)
    # time.sleep(30000)
    # test_dirs = [
    #     '/home/ubuntu/HDD2T/HWK/helen_result/Helen_GAN_DIC',
