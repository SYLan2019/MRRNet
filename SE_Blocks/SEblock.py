...
class Inception_Module(nn.Module):  # try reflection pad
    ...
    def forward(self, x):
        ...
        avg_feature = self.avg(feature_fusion).squeeze(-1).squeeze(-1)
        # print(avg_feature.size(), self.out)
        fc1_feature = self.fc1(avg_feature)
        # time.sleep(20)
        relu_feature = self.relu(fc1_feature)
        fc2_feature = self.fc2(relu_feature)
        fc2_feature = fc2_feature.unsqueeze(-1).unsqueeze(-1)

        weight = self.sigmoid(fc2_feature)
        final = weight * identity
        return final
