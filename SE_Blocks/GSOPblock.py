...
class Inception_Module(nn.Module):  # try reflection pad
    def __init__(self, in_c, out_c):
        ...
        # for GSOP
        self.row_bn = nn.BatchNorm2d(out_c)
        self.row_conv_group = nn.Conv2d(out_c, 4 * out_c, kernel_size=(out_c, 1), groups=out_c,
                                        bias=True)
        self.fc_adapt_channels = nn.Conv2d(4 * out_c, out_c, kernel_size=1, groups=1, bias=True)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        out = MPNCONV.CovpoolLayer(feature_fusion)
